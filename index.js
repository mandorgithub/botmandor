...
cangtion = `
*Byee Byee...* @${num.split("@")[0]} 👋
*Kenapa Gak Dari Kemarin2 Aja Sih...*
` 
...
Left = await getBuffer(`https://leyscoders-api.herokuapp.com/api/leave?name=${nama}&group=${encodeURIComponent(metadata.subject)}&profile=${ppuser}&apikey=IkyOgiwara`)
                ...
                if (anu.action == 'add') {
                    kirbotz.sendButWel(anu.id, cangton, `© Created By Mandorrenville `, Welcome, vtbtn)
                } else if (anu.action == 'remove') {
                   kirbotz.sendButWel(anu.id, cangtion, `© Created By Mandorrenville `, Left, bkgc)
                }
            }
        } catch (err) {
            console.log(err)
